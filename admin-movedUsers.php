            ...
            <div class="col-9 padding-0">
                ...
                <table class="table table-striped">
                    ...
                    <tbody id="tbody">
                        ...
                        $sql = "SELECT user.id AS userID, resident.ID AS residentID, name, surname, birthDate, tel, email, username, isMoved AS residentUsername, username FROM user, resident WHERE user.residentID = resident.id AND isMoved = 1";
                        ...
                        while($row = $result->fetch_assoc()) {
                            ...
                            <tr>
                                ...
                                <td>'.$row['tel'].'</td>
                                <td>'.$row['email'].'</td>
                                <td>'. '<a class="btn btn-info" href="../ApartmentManager/admin-editUser.php?id='.$row['userID'].'&residentUsername='.$row['residentUsername'].'&name='.$row['name'].'&surname='.$row['surname'].'&birthDate='.$row['birthDate'].'&tel='.$row['tel'].'&email='.$row['email'].'">Edit</a>'.'</td>
                            </tr>
                            ';
                        }
